...
<div class="ds-page">
    <div class="ds-page__header">
        <div>
            ...
            <p class="ds-page__subtitle">Selecciona un paciente para crear una nueva cotización</p>
        </div>
        <div class="ds-page__actions">
            <a href="<?= base_url('/cotizaciones') ?>" class="ds-btn ds-btn--secondary">
    ...
    <?php if (!empty($pacientes)): ?>
        ...
        <div class="ds-grid" id="pacientesGrid" style="gap: 20px; display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr));">
            ...
            <div class="paciente-card" data-search="<?= strtolower(esc($paciente['nombre'] . ' ' . ($paciente['primer_apellido'] ?? '') . ' ' . ($paciente['identificacion'] ?? ''))) ?>">
                <div class="ds-card" style="height: 100%; transition: all 0.3s ease; cursor: pointer;" onmouseover="this.style.transform='translateY(-5px)'; this.style.boxShadow='0 10px 30px rgba(0,0,0,0.15)'" onmouseout="this.style.transform='none'; this.style.boxShadow=''">
                    ...
                    <div style="border-top: 1px solid #eee; padding: 15px; text-align: center;">
                        <a href="<?= base_url('/cotizaciones/crear/' . $paciente['id']) ?>" class="ds-btn ds-btn--primary ds-btn--full">
                            <span class="ds-btn__icon">💵</span> Crear Cotización
                        </a>
                    </div>
                </div>
            </div>
    ...
    <?php else: ?>
        <div class="ds-empty-state" style="padding: 60px 20px;">
            ...
            <p class="ds-text-muted">Primero debes crear pacientes antes de poder crear cotizaciones.</p>
